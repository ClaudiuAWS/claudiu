...
def get_match_events(match_id: str) -> list:
    ...
    Note: gameTimes are returned UNCHANGED from the loader output. The
    15-minute halftime break lives in reveal/firing wall-clock space, not
    in the event data — see `useMatch.js` and `replay-emitter/service.py`.
    """
    events = []
    kwargs = {
